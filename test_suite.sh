...
api_subscribe() {
  ...
  local topic="$2"
  local threshold="$3"
  curl -s -X POST "$GATEWAY_URL/api/subscribe" \
...
api_publish() {
  ...
  local topic="$2"
  local message="$3"
  curl -s -X POST "$GATEWAY_URL/api/publish" \
    -H "Content-Type: application/json" \
...
resp=$(curl -s -X POST "$GATEWAY_URL/api/subscribe" -H "Content-Type: application/json" -d 'invalid json')
...
if command -v wscat >/dev/null 2>&1; then
  ...
  if [[ -n "$TIMEOUT_CMD" ]]; then
    output=$($TIMEOUT_CMD 5 wscat -c "ws://$GATEWAY_URL/api/ws?client_id=$CLIENT_ID" 2>&1)
...
resp=$(curl -s -X POST "$GATEWAY_URL/api/publish" \
  -H "Content-Type: application/json" \
